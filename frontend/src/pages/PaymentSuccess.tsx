...
export default function PaymentSuccess() {
  ...
  const { selectedPlan, applySelectedPlanToRole } = useDemoState();
      ...
      <div className="max-w-md w-full bg-white rounded-xl shadow-2xl shadow-primary/5 border border-outline-variant/30 p-10 text-center relative z-10 space-y-8 transform transition-all animate-in fade-in zoom-in duration-500">
        ...
        <div className="space-y-3">
          ...
          <p className="text-on-surface-variant text-sm leading-relaxed">
            Your transaction has been securely processed and your account has
            been updated. You can now access your new features.
          </p>
          {selectedPlan && (
            <p className="text-xs font-bold text-secondary uppercase tracking-wider">
              Activated: {selectedPlan === "pro" ? "Pro" : "Developer"} Plan
            </p>
          )}
        </div>
