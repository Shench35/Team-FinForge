...
export default function Developer() {
  ...
  useEffect(() => {
    if (
      localStorage.getItem("demo_developer_paid") === "true" &&
      user?.email &&
      !apiKey
    ) {
      ...
      <DashboardLayout>
        <Card className="p-8 max-w-2xl space-y-4">
          <h1 className="text-2xl font-bold text-primary">
            Developer Portal Locked
          </h1>
          <p className="text-sm text-on-surface-variant">
            Upgrade to the Developer plan from your Dashboard paywall to unlock
            API keys and B2B credits.
          </p>
          <Button
            variant="primary"
          ...
          <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              <div className="space-y-1">
                ...
                <p className="text-sm text-on-surface-variant">
                  Use this key to authenticate your requests to the FinForge
                  Verification API.
                </p>
              </div>
              {!apiKey && (
          ...
          <Card className="p-8 space-y-8 bg-white">
            ...
            <div className="space-y-4">
              ...
              <p className="text-sm text-on-surface-variant leading-relaxed">
                All requests to the Verification API must include your API key
                in the{" "}
                <code className="bg-surface-container px-1.5 py-0.5 rounded text-primary text-xs">
                  x-api-key
                </code>{" "}
                header.
              </p>
            ...
            <div className="space-y-6">
              ...
              <p className="text-sm text-on-surface-variant leading-relaxed">
                The core verification engine. Analyzes a document and returns
                authenticity scores and extracted data.
              </p>

              <div className="space-y-2">
                <div className="flex items-center gap-3 text-sm">
                  <span className="bg-secondary/10 text-secondary font-bold px-2 py-1 rounded text-xs">
                    POST
                  </span>
                  <code className="font-mono text-on-surface bg-surface-container px-2 py-1 rounded">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-2">
                ...
                <div className="space-y-2">
                  ...
                  <ul className="text-sm text-on-surface space-y-2">
                    <li>
                      <code className="text-xs font-bold">file</code>: Image
                      (JPG/PNG) or PDF
                    </li>
                    <li>
                      <code className="text-xs font-bold">cert_type</code>:{" "}
                      <span className="italic">WAEC</span> or{" "}
                      <span className="italic">NECO</span>
                    </li>
                  </ul>
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="bg-[#0F172A] p-4 rounded-md overflow-x-auto">
                  <pre className="text-[13px] text-gray-300 font-mono leading-relaxed">
                    <span className="text-pink-400">const</span> axios =
                    require(<span className="text-green-300">'axios'</span>);
                    <span className="text-pink-400">const</span> FormData =
                    require(<span className="text-green-300">'form-data'</span>
                    );
                    <span className="text-pink-400">const</span> fs = require(
                    <span className="text-green-300">'fs'</span>);
                    <span className="text-pink-400">const</span> form ={" "}
                    <span className="text-pink-400">new</span> FormData();
                    form.append(<span className="text-green-300">'file'</span>,
                    fs.createReadStream(
                    <span className="text-green-300">'certificate.pdf'</span>));
                    form.append(
                    <span className="text-green-300">'cert_type'</span>,{" "}
                    <span className="text-green-300">'WAEC'</span>); axios.post(
                    <span className="text-green-300">
                      'http://api.finforge.com/third_party/api/v1/verify'
                    </span>
                    , form, {"{"}
                    headers: {"{"}
                    ...form.getHeaders(),
                    <span className="text-green-300">'x-api-key'</span>:{" "}
                    <span className="text-green-300">'cvfy_your_key'</span>
                    {"}"}
